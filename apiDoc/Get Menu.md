...
Query Parameters
Name	Description
menu_type	(optional) Menu type to return.

ALLOWED VALUES:
MENU_TYPE_FULFILLMENT_DELIVERY
MENU_TYPE_FULFILLMENT_PICK_UP
MENU_TYPE_FULFILLMENT_DINE_IN
Defaults to MENU_TYPE_FULFILLMENT_DELIVERY if not provided
Response Body Parameters
Response Body Parameters - MenuConfiguration
...
title	MultiLanguageText	The name of the menu to be displayed (by locale).
subtitle	MultiLanguageText	An optional subtitle for the menu (by locale).
service_availablity	ServiceAvailability[]	The days and times of the day at which this menu should be made available.
category_ids	string[]	All of the IDs for the menu categories that will be made available while this menu is active.
Response Body Parameters - MultiLanguageText
Provides content for a string displayed to users in multiple languages.

Name	Type	Description
translations	Object (key string: value string)	A mapping from locale code to the translated text in each locale. One translation must be provided. Currently, we do not support additional translations. When displaying text, we will first attempt to use the translation for the locale used by the user’s device, then, if not found, the restaurant’s locale, and finally the default locale for the city. If none of these are found, the first locale listed will be used.

Locale codes should specify both language and country codes, e.g. “en_us”
Response Body Parameters - Service Availability
...
Response Body Parameters - TimePeriod
...
title	MultiLanguageText	The displayed name for the category (by locale).
subtitle	MultiLanguageText	(optional) A subtitle for the category (by locale).
entities	MenuEntity[]	The top-level menu items available for sale within the category - all entities must be of type “ITEM”.
Response Body Parameters - MenuEntity
...
external_data	string	(optional) Free-form text field reserved for the restaurant’s use, e.g. for POS integrations.
title	MultiLanguageText	The name of the item (by locale).
description	MultiLanguageText	(optional) Supplementary information describing the item (by locale).
...
Price should always be integer value (never decimals) because the price is set in percent of local currency denomination (e.g., cents for US currency, not dollars).
...
Response Body Parameters - PriceRules
Specifies the price to charge for ordering the item.

Name	Type	Description
price	int	Price of the item in the percent of local currency denomination, e.g. cents for dollar and 1/100 for yen.
...
Response Body Parameters - PriceOverride
...
Can be set for Items and Modifier Options but currently only used for determining value of Modifier Options.
Response Body Parameters - QuantityConstraintRules
...
Response Body Parameters - QuantityConstraint
...
Response Body Parameters - QuantityConstraintOverride
...
Response Body Parameters - SuspensionRules
...
Response Body Parameters - Suspension
...
Response Body Parameters - SuspensionOverride
...
Response Body Parameters - ModifierGroupsRules
...
Response Body Parameters - ModifierGroupsOverride
...
Response Body Parameters - TaxInfo
...
Response Body Parameters - DietaryLabelInfo
Dietary labels visible to the user.

Name	Type	Description
labels	string[] (DietaryLabels)	(optional) A list of strings from the allowed DietaryLabels table below which are applicable for the item/modifier option.
Response Body Parameters - DietaryLabels
...
Response Body Parameters - VisibilityInfo
...
Response Body Parameters - visibilityHours
Specifies when an item is visible to the user
...
Response Body Parameters - HoursOfWeek
...
external_data	string	(optional) Free-form text field reserved for the restaurant’s use, e.g. for POS integrations.
title	MultiLanguageText	The displayed name of the modifier group (by locale).
...
Response Body Parameters - ProductInfo
...
Response Body Parameters - BundledItems
...
Response Body Parameters - SellingInfo
...
Response Body Parameters - SellingOption
...
Response Body Parameters - MeasurementUnit
...
Response Body Parameters - SellingQuantityConstraint
...
Response Body Parameters - PricedByToSoldByUnitConversionInfo
